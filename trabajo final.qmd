...
modelo_arbol <- rpart(popularity ~ ., data = train, method = "class",
                       control = rpart.control(cp = 0.005))

rpart.plot(modelo_arbol, type = 3, extra = 101, fallen.leaves = TRUE,
           main = "Árbol de decisión - Popularidad")

#Evaluamos en los datos de prueba 

pred_arbol <- predict(modelo_arbol, newdata = test)
rmse_arbol <- sqrt(mean((test$popularity - pred_arbol)^2))
cat("RMSE árbol de decisión:", round(rmse_arbol, 2), "\n")


```

```{r}
#Evaluamos con el modelo de random forest
